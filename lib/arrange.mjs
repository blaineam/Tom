...
// same seed = the same audio, byte for byte.
import {
...
export function render(bp, { onProgress } = {}) {
  ...
  const ctx = {
    ...
    drum(name, t, vel = 1, opts = {}) {
      ...
      if (!sig) {
        sig = name === 'kick' ? I.kick(drumRng)
          : name === 'snare' ? I.snare(drumRng, opts.tone ?? 190, 1, opts.tail ?? 0.12)
          : name === 'clap' ? I.clap(drumRng)
          : name === 'hat' ? I.hat(drumRng, 1, !!opts.open)
          : name === 'shaker' ? I.shaker(drumRng)
          : name === 'crash' ? I.crash(drumRng)
          : null;
        if (!sig) throw new Error(`Unknown drum ${name}`);
  ...
  bp.blocks.forEach((b, bi) => {
    ...
    for (const span of spans) {
      ...
      if (L.bass) style.bass(ctx, ch, ts, span.beats, beat, energy);
    }
    for (let k = 0; k < b.bars; k++) {
      const tb = t0 + k * 4 * beat;
      if (L.drums === 'build') buildRoll(ctx, tb, beat, k, b.bars);
    ...
    if (L.lead || L.bells || L.counter) {
      ...
      for (const n of mel) {
        ...
        if (L.bells) ctx.play('bells', 'bell', n.midi, t, I.bell(n.midi, d + 0.6), style.bellGain ?? 0.6, 0.1, 'leads');
      }
      if (L.counter) {
  ...
  const [L, Rr] = reverb(main.L, main.R, { size: 0.7, mix: 0.15 });
...
function ending(ctx, bp, { style, S, root, beat }, t, tail) {
  ...
  [...tonic, tonic[0] + 12].forEach((m, k) => {
    ctx.play('bells', 'bell', m + 12, t + 0.35 + k * beat / 2, I.bell(m + 12, 2.0), 0.4, 0.3 - k * 0.2, 'leads');
  });
}
